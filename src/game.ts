...
{
    depart: number;
    move_1: number; // jusqu'a moitie / (2/3) de la traj et a + ou - 20/30 pixels d'un des bords du paddle
    move_2: number; // avec les erreurs lies au deplacement
    counter: number;
    rebond: number;
...
function display_time(ctx: CanvasRenderingContext2D, paddle: game_config): void
{
    if (!ctx)
        return;
...
class Pong
{
    ...
    game_loop(): void
    {
        if (!this.state.is_paused)
        {
            ...
            {
                this.ia.counter++;
                //if (this.ia.counter >= this.ball.time_ia_in_frame)
                //        console.log("LAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
                if (this.ia.random_paddle_move == true && this.ia.rebond != 0)
                    this.update_paddle_ia_with_rebond();
                else
    ...
    update_paddle_ia_with_rebond()
    {
        if (this.ia.depart >= this.ia.counter) 
            return ;
        if (this.ball.current_rebond < this.ia.rebond)
        {
            if (this.ia.ia_debug == true)
            {
                console.log("****** move 1 *********");
    ...
    ia_ajustement(marge: number, random: boolean)
    {
        ...
        // if (Math.abs(distance) <= marge && random == true && this.ia.move_flag == true) // pas oublier de rajouter random
        // {
        //     if (this.ia.continue_flag == true)
        //     {
        //         //console.log(" ----- CONTINUE FLAG ----- ");
        //         this.ia.distance_with_marge = center_paddle - this.ball.ia_y // dist < 0 pour paddle au dessus de la balle
    ...
    // continue_movement(marge: number)
    // {   
    //     let center_paddle = this.paddle.right_paddle_y + this.config.paddle_height / 2;
    //     let target_y = this.ball.ia_y;

    //     if (center_paddle < target_y)
    //         target_y += (marge * 0.8);
    //     else
    //         target_y -= (marge * 0.8);
...
export class Game
{
    ...
    start_game_loop(mode: 'solo' | 'versus'): void
    {
